...
class PEBuilder:
  ...
  def build_exe(code, data, entry_point):
    ...
    pe.extend(struct.pack('<H', 0x014C))  # 机器类型 (x64)
    pe.extend(struct.pack('<H', 2))       # 节数量
    pe.extend(b'\x00' * 8)                # 时间戳等
    pe.extend(struct.pack('<L', 0x00E0))  # 可选头大小
    pe.extend(struct.pack('<H', 0x010F))  # 特性
        
    # 可选头
    pe.extend(struct.pack('<H', 0x010B))  # 魔术数 (PE32+)
    ...
    pe.extend(b'\x00' * 16)
    pe.extend(struct.pack('<L', 0x00401000)) # 特性
    pe.extend(code)
        
    # 数据节
    pe.extend(b'.data\x00\x00\x00')
    pe.extend(struct.pack('<L', len(data)))  # 虚拟大小
    pe.extend(struct.pack('<L', 0x00402000))     # 虚拟地址
